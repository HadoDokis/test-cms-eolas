...
if (!empty($_GET['classname'])) {
    ...
    $aRow = $oObjet->getLiaisonUtilisateur($LIA_TYPE);

    if (is_numeric($_GET['insert'])) {
        if (!$withMax || ($withMax > count($aRow))) {
            $oObjet->saveLiaisonUtilisateur($_GET['insert'], $_GET['onlyOne'] ? $LIA_CODE : '', $LIA_TYPE, $LIA_TEXT);
            $aRow = $oObjet->getLiaisonUtilisateur($LIA_TYPE);
        } else {
        ...
        if (!$withMin || ($withMin < count($aRow))) {
            $oObjet->deleteLiaisonUtilisateur($_GET['delete'], $_GET['onlyOne'], $LIA_TYPE);
            $aRow = $oObjet->getLiaisonUtilisateur($LIA_TYPE);
        } else {
            $error = sprintf(gettext('selection liaisons minimum atteint'), $withMin);
        }
    } elseif (is_numeric($_GET['up'])) {
        $oObjet->upLiaisonUtilisateur($_GET['up']);
        $aRow = $oObjet->getLiaisonUtilisateur($LIA_TYPE);
    } elseif (is_numeric($_GET['down'])) {
        $oObjet->downLiaisonUtilisateur($_GET['down']);
        $aRow = $oObjet->getLiaisonUtilisateur($LIA_TYPE);
    }
    ...
    if (sizeof($aRow)>0) {?>
        <ul class="ajaxListe">
            ...
            $cpt = 1;
            foreach ($aRow as $row) { ?>
            <li id="<?php echo $LIA_CODE?>_<?php echo $row['ID_UTILISATEUR']?>" class="ajaxItem"><!-- Pour controle des formulaires min et max -->
                <?php if (!$withMin || (count($aRow)>$withMin)) { ?>
                <a href="#" onclick="ajaxLiaison.getAjax('<?php echo $divID ?>', 'utilisateur', 'delete', <?php echo $row['ID_UTILISATEUR']?>); return false;"><img src="<?php echo SERVER_ROOT;?>images/pagination/delete.gif" alt="<?php echo gettext('DELETE')?>"></a>
                <?php } ?>
                <?php if ($cpt != 1) { ?>
                <a href="#" onclick="ajaxLiaison.getAjax('<?php echo $divID ?>', 'utilisateur', 'up', <?php echo $row['ID_LIAISON_UTILISATEUR']?>); return false;"><img src="<?php echo SERVER_ROOT;?>images/pagination/triDesc.gif" alt="<?php echo gettext('Monter')?>"></a>
                <?php } else { ?>
                <img src="<?php echo SERVER_ROOT;?>images/pagination/empty.gif" alt="">
                <?php } ?>
                <?php if ($cpt != count($aRow)) { ?>
                <a href="#" onclick="ajaxLiaison.getAjax('<?php echo $divID ?>', 'utilisateur', 'down', <?php echo $row['ID_LIAISON_UTILISATEUR']?>); return false;"><img src="<?php echo SERVER_ROOT;?>images/pagination/triAsc.gif" alt="<?php echo gettext('Descendre')?>"></a>
                <?php } else { ?>
                <img src="<?php echo SERVER_ROOT;?>images/pagination/empty.gif" alt="">
                <?php } ?>
                <label><?php echo secureInput($row['UTI_NOM'] . ' ' . $row['UTI_PRENOM'] . '['. $row['ID_UTILISATEUR'] .']')?></label>
                <?php if ($hasText) { ?>
                <input type="text" id="<?php echo $divID?>_<?php echo $row['ID_UTILISATEUR']?>_text" value="<?php echo secureInput($row['LIA_TEXT'])?>" size="40" onchange="ajaxLiaison.saveText(<?php echo $row['ID_LIAISON_UTILISATEUR']?>, 'UTILISATEUR', this.value);return false;" placeholder="<?php echo secureInput($labelText)?>"<?php if($textIsNotNull) echo ' required';?>>
                <?php } ?>
            </li>
            <?php
                $cpt++;
    ...
    die('Vous devez faire référence au fichier appelant dans "ajax.liaison_utilisateur.php"');
}
