...
if ($p->getNb() > 0) {
    ...
    if (isset($_GET['export'])) {
        ...
        header("Content-Disposition:attachment; filename=\"Export_Formulaire_" . date('dmY',$dateDebut) . "_" . date('dmY',strtotime("-1 day", $dateFin)) . ".csv\"");
        $f = fopen("php://temp", 'w');
...
<table class="liste">
    <thead>
        <tr>
            <th><?php echo $p->tri('Formulaire', 'HID_INFO')?></th>
    ...
    <?php foreach ($aDataTotal as $key => $row) {?>
        <tr>
            <td><?php echo secureInput($aFormulaire[$key])?></td>
            <td class="aligncenter">
                <?php if ($row['ACTION']) {?>
                <a href="stat_formulairePopup.php?ID_FORMULAIRE=<?php echo $key?>&amp;dateDebut=<?php echo $dateDebut?>&amp;dateFin=<?php echo $dateFin?>" class="popup"><?php echo $row['ACTION']?></a>
                <?php } else {?>
                0
                <?php }?>
            </td>
