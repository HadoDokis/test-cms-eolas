...
if ($p->getNb() > 0) {
    ...
    if (isset($_GET['export'])) {
        ...
        header("Content-Disposition:attachment; filename=\"Export_Webotheque_" . date('dmY',$dateDebut) . "_" . date('dmY',strtotime("-1 day", $dateFin)) . ".csv\"");
        $f = fopen("php://temp", 'w');
...
<table class="liste">
    ...
    foreach ($aDataTotal as $key => $row) {?>
        <tr>
            <td><?php echo secureInput(Webotheque::$_aTraduction[$key])?></td>
            <td class="aligncenter">
                <?php if ($row['ACTION']) { ?>
                <a href="stat_webothequePopup.php?WBT_CODE=<?php echo $key?>&amp;dateDebut=<?php echo $dateDebut?>&amp;dateFin=<?php echo $dateFin?>" class="popup"><?php echo $row['ACTION']?></a>
                <?php } else {?>
                0
                <?php }?>
            </td>
