...
$filtre = ' h.SIT_CODE = ' . $dbh->quote(CMS::getCurrentSite()->getID()) . '
              and h.HIS_DATE >= ' . $date1erJour . '
              and h.HIS_DATE < ' . strtotime('midnight');

$sql = "select count(distinct ID_UTILISATEUR) from HISTORIQUE_UTILISATEUR h where " . $filtre;
$nb = $dbh->query($sql)->fetchColumn();
$aTotal['CONTRIBUTEUR']['CONNECTE'] = $nb;

$sql = 'select count(ID_UTILISATEUR) from UTILISATEUR where ID_UTILISATEUR in (select distinct ID_UTILISATEUR from ROLE where SIT_CODE = ' . $dbh->quote(CMS::getCurrentSite()->getID()) . ' or SIT_CODE is null)';
$nb = $dbh->query($sql)->fetchColumn();
...
<body>
<div id="document">
    <?php $aMenuKey = array('TDB', 'SSC'); include('../include/inc.bo_bandeau_haut.php') ?>
    <div id="corps">
        <div id="bo_contenu">
            <h2><?php echo secureInput(CMS::getCurrentSite()->getField('SIT_LIBELLE') . ' : ' . gettext('activite_tableaubord'));?></h2>
            <div class="creation">
                <fieldset class="demiLeft">
                    ...
                    <p class="alignright">
                        <a class="action" href="stat_utilisateur.php" title="statistiques contributeurs"><?php echo gettext('Voir plus');?></a>
                    </p>
                </fieldset>
                <fieldset class="demiRight">
                    ...
                    <p class="alignright clear">
                        <a class="action" href="stat_contenu.php" title="statistiques pages"><?php echo gettext('Voir plus');?></a>
                    </p>
                </fieldset>
                <fieldset class="demiLeft">
                    ...
                    <p class="alignright">
                        <a class="action" href="stat_contenu.php?type=webo" title="statistiques webothèque"><?php echo gettext('Voir plus');?></a>
                    </p>
                </fieldset>
                <fieldset class="demiRight">
                    ...
                    <p class="alignright">
                        <a class="action" href="stat_contenu.php?type=form" title="statistiques formulaires"><?php echo gettext('Voir plus');?></a>
                    </p>
                </fieldset>
                <?php if (Historique::isModuleActif()) {?>
                    <fieldset class="demiLeft">
                        ...
                        <p class="alignright">
                            <a class="action" href="stat_contenu.php?type=module" title="statistiques modules"><?php echo gettext('Voir plus');?></a>
                        </p>
                    </fieldset>
                <?php }?>
                <fieldset class="<?php echo Historique::isModuleActif() ? 'demiRight' : 'demiLeft';?>">
                    ...
                    <p class="legend"><?php echo intval($aTotal['ADMIN']['ACTION']['SITE']) . ' '; echo intval($aTotal['ADMIN']['ACTION']['SITE']) > 1 ?  gettext('actions_enregistrees') : gettext('action_enregistree') ; echo ' ' . gettext('sur_le_site')?></p>
                    <p class="legend"><?php echo intval($aTotal['ADMIN']['ACTION']['UTILISATEUR']) . ' '; echo intval($aTotal['ADMIN']['ACTION']['UTILISATEUR']) > 1 ? gettext('actions_enregistrees') : gettext('action_enregistree'); echo ' sur ' . intval($aTotal['ADMIN']['UTILISATEUR']) . ' ' . gettext('utilisateur'); echo intval($aTotal['ADMIN']['UTILISATEUR']) > 1 ? 's':''?></p>
                    <p class="alignright">
                        <a class="action" href="stat_contenu.php?type=admin" title="statistiques administration"><?php echo gettext('Voir plus');?></a>
                    </p>
                </fieldset>
            </div>
