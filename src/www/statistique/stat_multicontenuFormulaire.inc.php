<?php
$p = new Pagination();
$filtre = ' HID_TYPEDONNEE =  "FORMULAIRE"
...
if ($p->getNb() > 0) {
   ...
   $sqlTab = "select
                    ...
                    count(distinct h.HID_IDENTIFIANT) as FORMULAIRE,
    ...
    $ACTION = 0;
    $aDataAction = $aFormulaire = array();
    foreach ($dbh->query($sqlGraph) as $row) {

        if ($row['HID_TOTALACTION'] > 0) {
            ...
            $aFormulaire[$row['HID_IDENTIFIANT']] = $row['HID_INFO'];
        }
    }
    if (isset($_GET['export'])) {
        ...
        header("Content-Disposition:attachment; filename=\"Export_SiteFormulaire_" . date('dmY',$dateDebut) . "_" . date('dmY',strtotime("-1 day", $dateFin)) . ".csv\"");
        $f = fopen("php://temp", 'w');
...
<script type="text/javascript">
    ...
    <?php if (intval($ACTION) > 0) {?>
        ...
        <?php  foreach ($aDataAction as $type => $row) { ?>
            ...
            var plot = {
                data: d,
                label: "<?php echo secureInput(ucfirst($aFormulaire[$type]));?>",
        ...
        <table class="liste">
            <thead>
                <tr>
                    ...
                    <th><?php echo $p->tri(gettext('nb_formulaires'), 'FORMULAIRE')?></th>
           ...
           <?php foreach ($p->fetch($sqlTab, 'h.SIT_CODE') as $row) {?>
                <tr>
                    ...
                    <?php if (intval($row['ACTION']) > 0) {?>
                            <a href="stat_sitePopup.php?SIT_CODE=<?php echo $row['SITE']?>&amp;type=FORM&amp;dateDebut=<?php echo intval($dateDebut)?>&amp;dateFin=<?php echo intval($dateFin)?>#fieldset_3" class="popup">
                    ...
                    <td class="aligncenter"><?php echo intval($row['FORMULAIRE']);?></td>
