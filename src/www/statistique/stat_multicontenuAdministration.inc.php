...
if ($p->getNb() > 0) {
   ...
   $sqlTab = "select
    ...
    $ACTION = 0;
    $aDataAction = $aActionUtilisateur = $aActionSite = array();
    foreach ($dbh->query($sqlGraph) as $row) {
        ...
        $aModule[$row['HID_IDENTIFIANT']] = extraireLibelle($row['HID_INFO']);
        if ($row['HID_IDENTIFIANT'] == 'SITE') {
            $aActionSite[$row['SIT_CODE']] = intval($aActionSite[$row['SIT_CODE']]) + intval($row['HID_TOTALACTION']);
        } else {
            $aActionUtilisateur[$row['SIT_CODE']] = intval($aActionUtilisateur[$row['SIT_CODE']]) + intval($row['HID_TOTALACTION']);
        }
    }
    ...
    if (isset($_GET['export'])) {
        ...
        header("Content-Disposition:attachment; filename=\"Export_SiteConfiguration_" . date('dmY',$dateDebut) . "_" . date('dmY',strtotime("-1 day", $dateFin)) . ".csv\"");
        $f = fopen("php://temp", 'w');
        ...
        $data = array(
                    gettext('Site'),
                    gettext('nb_actions'),
                    gettext('nb_actions_utilisateurs'),
                    gettext('nb_actions_sites'),
        ...
        foreach ($p->fetch($sqlTab, 'h.SIT_CODE', false) as $row) {
            $data = array(
                $row['INFO'],
                $row['ACTION'],
                intval($aActionUtilisateur[$row['SITE']]),
                intval($aActionSite[$row['SITE']]),
...
<form method="get" action="<?php echo PHP_SELF?>" class="filtre" id="form_stat">
    <table>
        <tr>
            <td class="aligncenter"><?php echo intval($ACTION) . ' '; echo intval($ACTION) > 1 ?  gettext('actions_enregistrees') : gettext('action_enregistree');?></td>
        </tr>
    </table>
    <hr>
    <table>
        ...
        <table class="liste">
            <thead>
                <tr>
                    ...
                    <th><?php echo gettext('nb_actions_utilisateurs')?></th>
                    <th><?php echo gettext('nb_actions_sites')?></th>
            ...
            <?php foreach ($p->fetch($sqlTab, 'h.SIT_CODE') as $row) {?>
                <tr>
                    ...
                    <?php if (intval($row['ACTION']) > 0) {?>
                            <a href="stat_sitePopup.php?SIT_CODE=<?php echo $row['SITE']?>&amp;type=ADMIN&amp;dateDebut=<?php echo intval($dateDebut)?>&amp;dateFin=<?php echo $dateFin?>#fieldset_5" class="popup">
                    ...
                    <td class="aligncenter"><?php echo intval($aActionUtilisateur[$row['SITE']])?></td>
                    <td class="aligncenter"><?php echo intval($aActionSite[$row['SITE']])?></td>
