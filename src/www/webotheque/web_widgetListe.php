...
CMS::checkAccess(new Module('MOD_WEBOTHEQUE_WIDGET'), array('PRO_WEBWIDGET', 'PRO_WEBROOT'));
...
<body>
<div id="document">
    <?php $aMenuKey = array('WEB', 'MOD_WEBOTHEQUE_WIDGET', 'WIDGET', 'LISTE'); include('../include/inc.bo_bandeau_haut.php') ?>
    <div id="corps">
        <div id="bo_contenu">
            <h2><?php echo gettext('Widgets')?></h2>
                ...
                <table class="liste">
                    <thead>
                        <tr>
                            ...
                            <th>
                                ...
                                {
                                    var tab = document.getElementsByName('del_web[]');
                                    for (i=tab.length-1; i>=0; i--) {
                                        if (!tab[i].disabled) {
                                            tab[i].checked = document.getElementById('del_web_all').checked;
                                        }
                                    }
                                }
                                </script>
                                <input type="checkbox" id="del_web_all" value="" onclick="toogleSelectAll();">
                            </th>
                            <?php } ?>
                    ...
                    foreach ($p->fetch($sql, 'WEBOTHEQUE.ID_WEBOTHEQUE') as $rowListe) {?>
                        <tr>
                            ...
                            <td><a href="web_widget.php?idtf=<?php echo $rowListe['ID_WEBOTHEQUE']?>"><?php echo secureInput($rowListe['WEB_LIBELLE'])?></a></td>
