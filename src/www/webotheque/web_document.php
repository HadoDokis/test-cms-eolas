...
CMS::checkAccess(new Module('MOD_WEBOTHEQUE_DOCUMENT'), array('PRO_WEBDOCUMENT', 'PRO_WEBROOT'));
...
<body>
<div id="document">
    <?php $aMenuKey = array('WEB', 'MOD_WEBOTHEQUE_DOCUMENT', 'DOCUMENT'); if (!$oWebotheque->exist()) $aMenuKey[]='ADD'; include('../include/inc.bo_bandeau_haut.php') ?>
    <div id="corps">
        <div id="bo_contenu">
            <h2><?php echo $oWebotheque->exist() ? secureInput($row['WEB_LIBELLE']) : 'Nouveau document' ?></h2>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="<?php echo UPLOAD_DOCUMENT . $row['WEB_CHEMIN'] ?>" onclick="window.open(this.href); return false;" class="action"><?php echo gettext('Telecharger fichier')?></a>
                            ...
                            <tr>
                                <th>
                                    <label for="WEB_CHEMINACC"><?php echo gettext('Alternative accessible')?></label>
                                    <div class="helper">
                                        <p><?php echo gettext('taille maximum') . ' : ' . File_management::getMaxUpload()?></p>
                                        <p><?php echo gettext('Extensions autorisees') . ': ' . implode(', ', Webotheque::getExtension($oWebotheque->code, true))?></p>
                                    </div>
                                </th>
                                <td>
                                    <input type="file" name="WEB_CHEMINACC" id="WEB_CHEMINACC" size="20">
                                    <?php if ($row['WEB_CHEMINACC'] != '') {?>
                                    <a href="<?php echo UPLOAD_DOCUMENT . $row['WEB_CHEMINACC'] ?>" onclick="window.open(this.href) ;return false;" class="action"><?php echo gettext('Telecharger fichier')?></a>
                                    (<?php echo gettext('Poids') . ': ' .  File_management::displayFileSize($row['WEB_TAILLEACC'])?>)
                                    <br><input type="checkbox" id="WEB_CHEMINACC_DELETE" name="WEB_CHEMINACC_DELETE" value="1">
                                    <label for="WEB_CHEMINACC_DELETE"><?php echo gettext('Supprimer')?></label>
                                    <?php } ?>
                                </td>
                            </tr>
                            <?php } ?>
