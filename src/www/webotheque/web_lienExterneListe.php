...
CMS::checkAccess(new Module('MOD_WEBOTHEQUE_LIENEXTERNE'), array('PRO_WEBLIENEXTERNE', 'PRO_WEBROOT'));
...
if ($p->onSearch()) {
    if (!empty ($_GET['WEB_LIBELLE'])) {
        $filtre .= " and (WEB_LIBELLE" . $p->makeLike('WEB_LIBELLE') . " or WEB_DESCRIPTION" . $p->makeLike('WEB_LIBELLE') . " or WEB_CHEMIN" . $p->makeLike('WEB_LIBELLE') . ")";
    }
...
<body>
<div id="document">
    <?php $aMenuKey = array('WEB', 'MOD_WEBOTHEQUE_LIENEXTERNE', 'LIENEXTERNE', 'LISTE'); include('../include/inc.bo_bandeau_haut.php') ?>
    <div id="corps">
        <div id="bo_contenu">
            <h2><?php echo gettext('Liens externes')?></h2>
            <form method="get" action="<?php echo PHP_SELF?>" class="filtre">
                <fieldset>
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                <th><label for="WEB_LIBELLE"><?php echo gettext('Libelle') . ' / ' . gettext('Description') . ' / ' . gettext('URL')?></label></th>
                ...
                <table class="liste">
                    ...
                    foreach ($p->fetch($sql, 'WEBOTHEQUE.ID_WEBOTHEQUE') as $rowListe) {?>
                        <tr>
                            ...
                            <td><a href="web_lienExterne.php?idtf=<?php echo $rowListe['ID_WEBOTHEQUE']?>"><?php echo secureInput($rowListe['WEB_LIBELLE'])?></a></td>
                            <td class="aligncenter"><a href="<?php echo secureInput($rowListe['WEB_CHEMIN'])?>" onclick="window.open(this.href);return false;" class="action"><?php echo gettext('Voir')?></a></td>
