...
<body>
<div id="document">
    <?php $aMenuKey = array('WEB', 'MOD_WEBOTHEQUE_LIENEXTERNE', 'LIENEXTERNE'); if (!$oWebotheque->exist()) $aMenuKey[]='ADD'; include('../include/inc.bo_bandeau_haut.php') ?>
    <div id="corps">
        <div id="bo_contenu">
            <h2><?php echo $oWebotheque->exist() ? secureInput($row['WEB_LIBELLE']) : 'Nouveau lien externe' ?></h2>
            <form method="post" action="web_webothequeSubmit.php" class="creation">
                <fieldset>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th><label for="WEB_CHEMIN"><?php echo gettext('URL')?></label></th>
                                <td>
                                    <input type="text" name="WEB_CHEMIN" id="WEB_CHEMIN" value="<?php echo secureInput($row['WEB_CHEMIN'])?>" size="80" required>
                                    <?php if ($row['WEB_CHEMIN'] != '') { ?>
                                    <a href="<?php echo secureInput($row['WEB_CHEMIN'])?>" onclick="window.open(this.href);return false;" class="action"><?php echo gettext('Voir')?></a>
                                    <?php } ?>
                                </td>
                            </tr>
                            <?php if (WEB_DESCRIPTION) { ?>
