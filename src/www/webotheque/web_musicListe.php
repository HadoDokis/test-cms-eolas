...
<body>
<div id="document">
    <?php $aMenuKey = array('WEB', 'MOD_WEBOTHEQUE_MUSIC', 'MUSIC', 'LISTE'); include('../include/inc.bo_bandeau_haut.php') ?>
    <div id="corps">
        <div id="bo_contenu">
            <h2><?php echo gettext('Audios')?></h2>
            ...
            <form method="post" action="web_webothequeSubmit.php">
                <table class="liste">
                    ...
                    foreach ($p->fetch($sql, 'WEBOTHEQUE.ID_WEBOTHEQUE') as $rowListe) {
                        $oWebo = new Webo_MUSIC($rowListe['ID_WEBOTHEQUE']);
                        ...
                        <tr>
                            ...
                            <td><a href="web_music.php?idtf=<?php echo $rowListe['ID_WEBOTHEQUE']?>"><?php echo secureInput($rowListe['WEB_LIBELLE'])?></a></td>
                            <td class="alignright"><?php echo File_management::displayFileSize($rowListe['WEB_TAILLE']) ?></td>
                            <td class="aligncenter"><?php echo $oWebo->getHTML()?></td>
